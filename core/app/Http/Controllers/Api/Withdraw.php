...
trait Withdraw{
    ...
    public function withdrawPreview($trx){  
        $withdraw = Withdrawal::with('method', 'user')->where('trx', $trx)->where('status', 0)->orderBy('id', 'desc')->first();
        if(!$withdraw){
    ...
    public function withdrawMoney(Request $request){   
        ...
        $method = WithdrawMethod::where('id', $request->method_id)->where('status', 1)->whereJsonContains('user_guards', "$guard")->first();
        if (!$method) {
            return response()->json([
                'remark'=>'validation_error',
                'status'=>'error',
    ...
    public function withdrawSubmitDone($actionId){    
        ...
        $userAction = UserAction::where('user_id', $user->id)->where('user_type', $userType)->where('is_api', 1)->where('used', 0)->where('id', $actionId)->first();
        if(!$userAction){
            return response()->json([
                'remark'=>'validation_error',
                'status'=>'error',
