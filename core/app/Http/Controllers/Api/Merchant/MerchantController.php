...
class MerchantController extends Controller
{
    ...
    {      
        ...
        $wallets = Wallet::hasCurrency()->where('user_id', $user->id)->where('user_type', 'MERCHANT')
            ->select(DB::raw('*'))
            ->addSelect(DB::raw('
                (select count(*) 
                from transactions
    ...
    { 
        ...
        $operations = ['make_payment', 'withdraw_money', 'add_balance', 'sub_balance'];
        $times = ['7days', '15days', '1month', '1year'];

        $allCurrency = auth()->user('merchant')->wallets; 
        $currencies = [];
    ...
    public function qrCodeDownload(){
        ...
        $filename = 'merchant_qr_code_'.$user->id . '.jpg';
    ...
    public function wallets(){ 
        $notify[] = "All Wallets";
        $wallets = Wallet::hasCurrency()->where('user_id', auth()->user('merchant')->id)->where('user_type', 'MERCHANT')
