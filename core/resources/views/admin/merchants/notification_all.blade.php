        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mt-3">
                    ...
                    <p>@lang('Email sent') <span class="sent">0</span> @lang('merchants out of') {{ $merchants }} @lang('merchants')</p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    (function($){
        ...
        function postMail(form,skip){
            ...
            $.post("{{ route('admin.merchants.notification.all.send') }}", {
                "subject":subject,
                "_token":_token,
                "skip":skip,
                "message":message
                ...
                if(response.error){
                    ...
                    var rest =  {{ $merchants }} - response.total_sent;
                    var sentPercent = response.total_sent / {{ $merchants }} * 100;
                    if (sentPercent > 100) {
                        sentPercent = 100;
                    }
                    ...
                    if(rest == 0){
                        setTimeout(() => {
                            ...
                            notify('success','Mail sent to all merchants successfully')
                        }, 3000);
                        return false;
                    }
                    setTimeout(()=>{
