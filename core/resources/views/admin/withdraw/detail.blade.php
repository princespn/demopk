                ...
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('User Type')
                            <span class="fw-bold">
                                {{__($withdrawal->user_type)}}
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Username')
                            <span class="fw-bold"> 
                                @php echo $withdrawal->goToUserProfile; @endphp
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Method')
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Amount')
                            <span class="fw-bold">{{ showAmount($withdrawal->amount,$withdrawal->curr) }} {{ __($withdrawal->curr->currency_code) }}</span>
                        </li>

                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Charge')
                            <span class="fw-bold">{{ showAmount($withdrawal->charge,$withdrawal->curr) }} {{ __($withdrawal->curr->currency_code) }}</span>
                        </li>

                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Payable')
                            <span class="fw-bold">{{ showAmount($withdrawal->final_amount,$withdrawal->curr) }} {{__($withdrawal->curr->currency_code)}}</span>
                        </li>
                ...
                <div class="card-body">
                    ...
                    @if($details != null)
                        @foreach(json_decode($details) as $val)
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <h6>{{__($val->field_name)}}</h6>
                                    @if($val->type == 'checkbox')
                                        {{ implode(',',$val->field_name ?? []) }}
                                    @elseif($val->type == 'file')
                                        @if($val->field_name)
                                            <a href="{{ route('admin.download.attachment',encrypt(getFilePath('verify').'/'.$val->field_name)) }}" class="me-3"><i class="fa fa-file"></i>  @lang('Attachment') </a>
                                        @else
                                            @lang('No File')
                                        @endif
                                    @else
                    ...
                    @if($withdrawal->status == 2)
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <button class="btn btn-outline--success ms-1 approveBtn" data-id="{{ $withdrawal->id }}" data-amount="{{ showAmount($withdrawal->final_amount,$withdrawal->curr) }} {{__($withdrawal->curr->currency_code)}}">
