          ...
          <div class="checkout-wrapper rounded-3  {{@$apiPayment->checkout_theme   == 'dark' ? 'checkout-wrapper--dark':''}} shake-card">
              ...
              @else
                ...
                <div class="checkout-wrapper__header text-center">
                  <img src="{{$apiPayment->site_logo}}" alt="image" class="form-logo mb-3">
                  <h3 class="product-price mt-2">{{@$apiPayment->currency->currency_symbol}} {{showAmount($apiPayment->amount,$apiPayment->currency)}} {{@$apiPayment->currency->currency_code}}</h3>
                  <h6 class="mb-5 title fw-normal mt-2">@lang('for') {{__($apiPayment->details)}}</h6>
                </div>
                <form class="mt-5 check-mail">
                    @csrf
                  <div class="form-group">
                    <input type="email" name="email" class="form--control" required placeholder="@lang('Enter your '.$general->site_name.' email address')" autocomplete="off">
                ...
                <div class="row mt-4">
                  <div class="col-6">
                    <p class="font-size--14px">@lang('Powered by') <a href="{{route('home')}}" class="text--secondary font-size--16px"><strong>{{$general->site_name}}</strong></a></p>
                  </div>
                  <div class="col-6">
